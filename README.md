...
## 아키텍처 의도적 배제: Retry & Circuit Breaker

본 프로젝트에서는 Resilience4j 등의 Retry 및 Circuit Breaker 라이브러리를 의도적으로 도입하지 않았습니다.

- 외부 의존성 없음: 현재 서비스는 외부 3rd-party API나 별도의 마이크로서비스(MSA) 연동 없이, 단일 내부 데이터베이스(RDB)와만 통신하는 모놀리식 구조입니다.

- 장애 전파 가능성 낮음: 내부 DB 커넥션 장애의 경우, 서킷 브레이커를 통한 Fallback 처리보다 커넥션 풀(HikariCP) 최적화 및 적절한 Timeout 설정이 본
  아키텍처에 더 적합하다고 판단했습니다.

- 오버 엔지니어링 방지: 인프라 복잡도를 낮추고 비즈니스 로직 구현에 집중하기 위해 과도한 장애 복구 메커니즘은 제외했습니다.
